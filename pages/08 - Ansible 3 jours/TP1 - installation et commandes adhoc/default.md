...
<!-- ```
ansible centos_hosts -m package -a name=epel-release state=present
``` -->
...
<!-- ```
ansible adhoc_lab --become --check -m systemd -a "name=nginx state=started" --check
``` -->
